...
- name: COOKIE_SECRET
  ...
  {{- end }}
- name: COOKIE_DOMAIN
  value: {{ tpl .Values.global.platform.console.host $ }}
{{- end -}}
...
- name: MEMBERSHIP_CLIENT_ID
  value: "{{ .Values.global.platform.console.oauth.client.id }}"
- name: MEMBERSHIP_CLIENT_SECRET
  {{- if gt (len .Values.global.platform.console.oauth.client.existingSecret) 0 }}
  valueFrom:
    secretKeyRef:
      name: {{ .Values.global.platform.console.oauth.client.existingSecret }}
      key: {{ .Values.global.platform.console.oauth.client.secretKeys.secret }}
  {{- else }}
  value: {{ .Values.global.platform.console.oauth.client.secret | quote }}
  {{- end }}
